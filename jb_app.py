...
def accept_jb_booking(index, booking_number, price, town):
    price = int(price)

    try:
        assert int(index) < int(len(st.session_state.jb_bookings))

        booking = st.session_state.jb_bookings[int(index)]
        ...
        if booking.language == "ENG":
            otl.jb_send_booking_confirmation_eng(booking)
        else:
            otl.jb_send_booking_confirmation_de(booking)
...
def reject_jb_booking(index):

    assert index < len(st.session_state.jb_bookings)

    booking = st.session_state.jb_bookings[index]
    try:
...
def reply_jb_quote(index, price):
    index = int(index)
    price = int(price)

    try:
        assert index < len(st.session_state.jb_quotes)

        quote = st.session_state.jb_quotes[index]
        ...
        if quote.language == "ENG":
            otl.jb_send_quote_eng(quote)
        elif quote.language == "DE":
            otl.jb_send_quote_de(quote)
        elif quote.language == "ES":
            otl.jb_send_quote_es(quote)
...
def decrease_jb_booking_index(len):
    st.session_state.jb_booking_index = (st.session_state.jb_booking_index - 1)%len

def increase_jb_booking_index(len):
    st.session_state.jb_booking_index = (st.session_state.jb_booking_index + 1)%len

def decrease_jb_quote_index(len):
    st.session_state.jb_quote_index = (int(st.session_state.jb_quote_index) - 1)%len

def increase_jb_quote_index(len):
    st.session_state.jb_quote_index = (int(st.session_state.jb_quote_index) + 1)%len
...
def show_jb_booking(variables, id):
...
def all_jb_bookings():
    ...
    for i in range(0, len(st.session_state.jb_bookings)):
        booking = st.session_state.jb_bookings[i]
...
def get_jb_bookings(jb_bookings):

    st.session_state.jb_bookings = otl.jb_manage_bookings()

def get_quotes():
    st.session_state.jb_quotes = otl.manage_quotes()

def jb_bookings() -> None:
    if "jb_accept_disabled" not in st.session_state:
        st.session_state["jb_accept_disabled"] = True
        st.session_state["jb_reject_disabled"] = True
    if "jb_booking_index" not in st.session_state:
        st.session_state["jb_booking_index"] = 0

    if "jb_bookings" not in st.session_state:
        st.session_state["jb_bookings"] = []
    show("jb_booking_hero")
    jb_bookings = []
    st.button("Get bookings", on_click= get_jb_bookings, args= (jb_bookings,))

    bookings_len = len(st.session_state["jb_bookings"])
    ...
    if bookings_len > 0:
        with right_left:
            st.button("  Previous", on_click = decrease_jb_booking_index, args= (bookings_len,))
            with right_right:
                st.button("Next ", on_click = increase_jb_booking_index, args = (bookings_len,))

        left,right = st.columns(2)

        with left:

            index = st.text_input("Booking index", value = st.session_state.jb_booking_index)
            ...
            if(inputs_filled(inputs)):
                st.session_state["jb_accept_disabled"] = False
            else:
                st.session_state["jb_accept_disabled"] = True

            if(index_input_filled(inputs)):
                st.session_state["jb_reject_disabled"] = False
            else:
                st.session_state["jb_reject_disabled"] = True

            st.button("Accept", on_click = accept_jb_booking, args = (index,booking_number,price,town), disabled = st.session_state.jb_accept_disabled)

            st.button("Reject", disabled = st.session_state.jb_reject_disabled, on_click = reject_jb_booking, args = (int(index),))
        with right:


            variables = [st.session_state.jb_booking_index, st.session_state.jb_bookings]
            show_jb_booking(variables, "booking")
...
def jb_quotes() -> None:
    ...
    if "jb_quotes" not in st.session_state:
        st.session_state["jb_quotes"] = []

    show("jb_quote_hero")
    st.button("Get quotes", on_click = get_quotes)

    len_quote = len(st.session_state.jb_quotes)
    st.write("We have found " + str(len_quote) + " quotes")

    show("spacer")

    if "jb_reply_disabled" not in st.session_state:
        st.session_state["jb_reply_disabled"] = True
    if "jb_quote_index" not in st.session_state:
        st.session_state["jb_quote_index"] = 0
    ...
    if len_quote > 0:
        ...
        with left_top:
            st.button("Previous", on_click = decrease_jb_quote_index, args = (len_quote,))
        with right_top:
            st.button("Next", on_click = increase_jb_quote_index, args = (len_quote,))

        main_left, main_right = st.columns(2)

        with main_left:
            quote_index = st.text_input("Quote index", value = st.session_state["jb_quote_index"])
            st.session_state["jb_quote_index"] = quote_index
            quote_price = st.text_input("Transfer price")

            quote_inputs = [quote_index, quote_price]

            if inputs_filled(quote_inputs):
                st.session_state.jb_reply_disabled = False
            else:
                st.session_state.jb_reply_disabled = True

            st.button("Reply", disabled = st.session_state["jb_reply_disabled"], on_click = reply_quote, args= (quote_index, quote_price,))

        with main_right:
            quote_variables = [int(st.session_state.jb_quote_index), st.session_state.jb_quotes]
