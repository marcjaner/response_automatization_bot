...
def preprocess(emails):
    ...
    for i in range(n_emails):
        ...
        # Talon function that extracts signature: check test.py
        email, _ = extract_signature(email)

        # Get all the lines from a specific email.
        lines = email.split('\n')

        # Iterate through all the lines
        for j in reversed(range(len(lines))):

            # deletes spaces at the beginning and end of a given line.
        ...
        # join lines into the email compartiment.
        emails[i] = ' '.join(lines)
...
def split_sentences(emails):
    ...
    for i in range(n_emails):
        ...
        # decomposes the email in sentences
...
def summarize(emails):
    ...
    """
    n_emails = len(emails)      # qty e-mails to treat.
